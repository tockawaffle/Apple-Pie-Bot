    ...
    async readFiles() {
        ...
        for (let fileData of [...files]) {
            ...
            if (type === "SLASH" || type === "BOTH") {
                ...
                if (testOnly) {
                    for (const guildId of this._instance.testServers) {
                        this._slashCommands.create(command.commandName, description, options, command.commandObject.descriptionLocalizations, command.commandObject.nameLocalizations, guildId);
                    }
                }
                else {
                    this._slashCommands.create(command.commandName, description, options, command.commandObject.descriptionLocalizations, command.commandObject.nameLocalizations);
                }
            }
        }
    }
