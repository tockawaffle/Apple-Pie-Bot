...
const disabled_commands_1 = __importDefault(
    require("./models/disabled-commands")
);
...
const channel_commands_1 = __importDefault(
    require("./models/channel-commands")
);
...
class CommandHandler {
    ...
    async setUp(
        ...
        for (const [file, fileName] of get_all_files_1.default(
        ...
        // Do not pass in TS here because this should always compiled to JS
        for (const [file, fileName] of get_all_files_1.default(
            path_1.default.join(__dirname, "command-checks")
        )) {
        ...
        if (dir) {
            ...
            console.log(
                `WOKCommands > Loaded ${amount} command${
                    amount === 1 ? "" : "s"
                }.`
            );
            ...
            client.on("messageCreate", async (message) => {
                ...
                try {
                    ...
                    if (error) {
                        ...
                        message.reply(
                            instance.messageHandler.get(guild, "EXCEPTION")
                        );
                        console.error(e);
                    }
                    instance.emit(
                        Events_1.default.COMMAND_EXCEPTION,
                        command,
                        message,
                        e
                    );
    ...
    async registerCommand(instance, client, file, fileName, builtIn = false) {
        ...
        if (slash && !(builtIn && !instance.isDBConnected())) {
            ...
            if (options.length) {
                ...
                let typesToUse;
                //Check which expectedArgsTypes is being used and use the enumeratedArgsTypes that matches it
                for(let i = 0; i < expectedArgsTypes.length; i++){
                    if(enumeratedArgsTypes[0][expectedArgsTypes[i]]){
                        typesToUse = enumeratedArgsTypes[0];
                        break;
                    }
                }
