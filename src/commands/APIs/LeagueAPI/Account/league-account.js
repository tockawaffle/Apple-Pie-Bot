...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        if(message.mentions.users.first()) {
            ...
            const maestryAmount = await fetch(new URL(`https://${checkReg.leagueReg}.api.riotgames.com/lol/champion-mastery/v4/scores/by-summoner/${checkReg.leagueEncSmId}?api_key=${process.env.riotkey}`))
            const usualInfo = await fetch(new URL(`https://${checkReg.leagueReg}.api.riotgames.com/lol/summoner/v4/summoners/by-name/${checkReg.leagueName}?api_key=${process.env.riotkey}`))
            const maestryChamps = await fetch(new URL(`https://${checkReg.leagueReg}.api.riotgames.com/lol/champion-mastery/v4/champion-masteries/by-summoner/${checkReg.leagueEncSmId}?api_key=${process.env.riotkey}`))
            const rankedInfo = await fetch(new URL(`https://${checkReg.leagueReg}.api.riotgames.com/lol/league/v4/entries/by-summoner/${checkReg.leagueEncSmId}?api_key=${process.env.riotkey}`))
