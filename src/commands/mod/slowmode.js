...
module.exports = {
    run: async(client, message) => {
        ...
        if(!time) {
            ...
            .addFields(
                {
                    name: `${languages(guild, "SM_C2")}`,
                    value: `${languages(guild, "SM_C3")}`
                },
                {
                    name: `${languages(guild, "SM_C4")}`,
                    value: `\`\`\`_slowmode <numberInSeconds> <#channelMention>\`\`\``
                }
            )
            .setColor('RED')
        ...
        if(time > 21600) {
            const tooHigh = new MessageEmbed()
                ...
                .addFields(
                    {
                        name: `${languages(guild, "SM_C6")}`,
                        value: `${languages(guild, "SM_C7")}`
                    }
                )
                .setColor('RANDOM')
        ...
        if(isNaN(time)) {
            ...
            .addFields(
                ...
                {
                    name: `${languages(guild, "SM_C12")}`,
                    value: `\`\`\`_slowmode <numberInSeconds>\`\`\``
                }
            )
            .setColor('RANDOM')
            const seconds = new MessageEmbed()
        ...
        const sucess = new MessageEmbed()
            ...
            .addFields(
                {
                    name: `Slowmode set to:`,
                    value: `\`\`\`${time} seconds\`\`\``
                },
                {
                    name: `Channel the slowmode was set:`,
                    value: `\`\`\`${chn.name}\`\`\``
                },
                {
                    name: `To remove the slowmode, type:`,
                    value: `\`\`\`_slowmode 0\`\`\``
                }
            ...
            chn.setRateLimitPerUser(time).catch(err => {
                ...
                const solution = new MessageEmbed()
                    ...
                    .addFields(
                        ...
                        {
                            name: `${languages(guild, "SM_C17")}`,
                            value: `${languages(guild, "SM_C18")}`
                        }
                    )
                pages = [
                    embedError,
