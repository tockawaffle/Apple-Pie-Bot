const langSchema = require('../../../configs/db/schemas/language-schema'); const { languages } = require('../../../util/languages/languages.json')
const { setLanguage } = require('../../../util/languages/languages'); const { MessageEmbed } = require('discord.js')
        ...
        if(message.member.hasPermission("MANAGE_GUILD")) {
            ...
            if(!targetLanguage) {
                const noargs = new MessageEmbed()
                    ...
                    `)
                    .addFields(
                        {name: 'You can choose between those languages:',value: '```_setl english or portugues```'},
                        {name: 'Você pode escolher entre esses idiomas:',value: "```_setl portugues ou english```"}
                    )
            ...
            const noPerm = new MessageEmbed()
                ...
                `)
                .addFields(
                    {name: 'Necessary Permission: || Permissão necessária:',value: '```Manage Guild || Gerenciar Servidor```'}
                )
                .setColor("RED")
