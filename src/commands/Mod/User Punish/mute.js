    ...
    run: async(client, message, args) => {
        try{
            ...
            if(!args[0]) {
                ...
                message.reply(invalidMember)
                return
            }else if(member.roles.cache.find(x => x.name === `${languages(guild, 'M_R')}`)) {
                const embed = new MessageEmbed()
                    .setDescription(`${languages(guild, 'M_C2')}`)
                    .setAuthor(guild.name, guild.iconURL({dynamic: true}))
                return message.reply(embed)
            }else if(!mutedRole && member) {
                const createdRole = await guild.roles.create({ data:{name: `${languages(guild, 'M_R')}`,color:"grey",permissions:[]}, reason:"Cargo de mutado!" })
                message.guild.channels.cache.forEach(async (channel, id) => { await channel.createOverwrite(await createdRole, { SEND_MESSAGES: false, SPEAK: false, READ_MESSAGES: false, ADD_REACTIONS: false }) })
                ...
                const embed = new MessageEmbed()
                    ...
                    .addFields(
                        ...
                        {name: languages(guild, "L_C2"), value: `\`\`\`${reason ? reason: languages(guild, "L_C12")}\`\`\``}
                    )
                    .setAuthor(`${guild.name}`, guild.iconURL({ dynamic: true }))
                    .setFooter(`${languages(guild, 'M4_C')} ${message.author.tag}`, message.author.displayAvatarURL({dynamic: true}));
                message.channel.send(embed).then((msg) => {
                    member.roles.add(createdRole).catch(err => {
                        msg.delete()
