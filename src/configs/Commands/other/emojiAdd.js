...
async function emojiAdd(messageCreate, input, name, author) {
    ...
    try {
        ...
        if(!getAttach) {
            throw new Error(`${lang(author, "no-args")}\n${lang(author, "emote-or-url")}`)
        } else if(!name && !getAttach) {
            throw new Error(`${lang(author, "missing-name")}`)
        } else if(!input) {
            throw new Error(`${lang(author, "no-args")}\n${lang(author, "emote-or-url")}`)
        }
        ...
        if(attachment) {
            ...
            emoji = input.match(/\d+/g)
            url = `https://cdn.discordapp.com/emojis/${emoji}.png`
            identifier = "Emoji JPG"
        } else if(input.match(/<a:.+?:\d+>/g)) {
            emoji = input.match(/\d+/g)
            url = `https://cdn.discordapp.com/emojis/${emoji}.gif`
            identifier = "Emoji GIF"
        } else if(input.match(/(https?:\/\/)?(cdn\.)?(discordapp\.com\/emojis)\/[a-zA-Z0-9_.]{18}(.gif|.png|.jpg|.jpeg).v=1/) || input.match(/(https?:\/\/)?(cdn\.|media\.)?(discordapp\.(com|net)\/attachments)\/[0-9]{18}\/[0-9]{18}\/[a-zA-Z0-9-_]+(.png|.gif|.jpeg|.jpg)/)) {
            identifier = "URL"
            url = input
        }
