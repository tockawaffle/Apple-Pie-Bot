async function getInfoWithURL(client, messageCreate, url) {
    ...
    const
        ...
        steam = client.steam;
    if(!url) throw new Error(lang(author, "ERROR_MISSING_URL"));
    try {
        await messageCreate.channel.sendTyping()
        let 
            steamResolve = await steam.resolve(url),
            steamSum = await steam.getUserSummary(steamResolve),
            steamBans = await steam.getUserBans(steamResolve),
            steamFriends = await steam.getUserFriends(steamResolve),
            steamLevel = await steam.getUserLevel(steamResolve),
            steamRecentGames = await steam.getUserRecentGames(steamResolve),
            steamOwned = await steam.getUserOwnedGames(steamResolve),
